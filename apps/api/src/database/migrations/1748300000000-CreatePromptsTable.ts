...
export class CreatePromptsTable1748300000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      INSERT INTO prompts (key, name, description, category, "promptType", "defaultLanguage", variables, "createdBy", "isActive") 
      SELECT 
        'task_solution_review_system' as key,
    ...
    await queryRunner.query(`
      INSERT INTO prompts (key, name, description, category, "promptType", "defaultLanguage", variables, "createdBy", "isActive") 
      SELECT 
        'solution_analysis_system' as key,
...
Provide detailed feedback on code quality, correctness, and adherence to best practices.' as content
      FROM prompts p WHERE p.key = 'task_solution_review_system';
    `);
...
4. Suggestions for improvement' as content
      FROM prompts p WHERE p.key = 'solution_analysis_system';
    `);
